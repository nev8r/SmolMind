...
from model.model_Smolmind import SmolMindConfig, SmolMindForCausalLM

warnings.filterwarnings('ignore', category=UserWarning)


# MoE模型需使用此函数转换
def convert_torch2transformers_Smolmind(torch_path, transformers_path, dtype=torch.float16):
    SmolMindConfig.register_for_auto_class()
    SmolMindForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = SmolMindForCausalLM(lm_config)
    ...
    print(f"模型已保存为 Transformers-SmolMind 格式: {transformers_path}")
...
if __name__ == '__main__':
    lm_config = SmolMindConfig(hidden_size=768, num_hidden_layers=16, max_seq_len=8192, use_moe=False)

    torch_path = f"../out/full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"

    transformers_path = '../SmolMind2'
