...
from model.SmolMind import SmolMindConfig, SmolMindForCausalLM
...
def init_model(args):
    if args.load == 0:
        tokenizer = AutoTokenizer.from_pretrained('../SmolMind/')
        moe_path = '_moe' if args.use_moe else ''
        modes = {0: 'pretrain', 1: 'full_sft(2048)', 2: 'dpo', 3: 'reason'}
        ckp = f'../{args.out_dir}/{modes[args.model_mode]}_{args.hidden_size}{moe_path}.pth'
        print(f"load the model from {ckp}")
        model = SmolMindForCausalLM(SmolMindConfig(
        ...
        model_path = '../SmolMind2'
        model = AutoModelForCausalLM.from_pretrained(model_path, trust_remote_code=True)
        tokenizer = AutoTokenizer.from_pretrained(model_path)
    print(f'SmolMind模型参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Server for SmolMind")
