...
def logits_to_log_probs(logits,labels):
    ...
    log_probs_per_token = torch.gather(
        log_probs,dim = -1,index=labels.unsqueeze(2)
    ).squeeze(-1)
    return log_probs_per_token

def dpo_loss(ref_log_probs,policy_log_probs,mask,beta):
    seq_lengths = mask.sum(dim=1,keepdim = True).clamp_min(1)
    ...
    logits = (chosen_policy_log_probs - chosen_ref_log_probs) - (reject_policy_log_probs - reject_ref_log_probs)
    
    return -F.logsigmoid(beta * logits).mean()


def train_epoch(epoch,loader,iters,ref_model,lm_config,start_step = 0,wandb = None):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SmolMind DPO (Direct Preference Optimization)")
    ...
    parser.add_argument('--from_weight', default='full_sft(2048)', type=str, help="基于哪个权重训练")
    ...
    parser.add_argument("--wandb_project", type=str, default="SmolMind-DPO", help="wandb项目名")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"SmolMind-DPO-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == start_epoch and start_step > 0: # 第一个epoch且存在检查点
            ...
            train_epoch(epoch, loader, len(loader) + start_step + 1, ref_model, lm_config, start_step, wandb)
        else: # 默认从头开始
            loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=(train_sampler is None), sampler=train_sampler, num_workers=args.num_workers, pin_memory=True)
            train_epoch(epoch, loader, len(loader), ref_model, lm_config, 0, wandb)